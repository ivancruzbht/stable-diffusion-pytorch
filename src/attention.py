...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, causal_mask=False) -> torch.Tensor:
        ...
        q,v,k = self.in_proj(x).chunk(3, dim=-1) # (B, C, T) -> (B, T, 3*C) -> 3*(B, T, C)
        ...
        output.transpose(1,2).contiguous().view(B, T, C)
        # (B, T, C) -> (B, T, C)
...
class CrossAttention(nn.Module):
    def __init__(self, n_heads: int, embed_size: int, cross_size: int, in_proj_bias=True, out_proj_bias=True):
        ...
        self.k_proj = nn.Linear(cross_size, cross_size, bias=in_proj_bias)
        self.v_proj = nn.Linear(cross_size, cross_size, bias=in_proj_bias)
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        B, T, C = x.size()
        interm_shape = (B, T, self.n_heads, self.head_size)
