...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor) -> torch.Tensor:
        ...
        x = self.groupnorm(x)
        x = self.conv_input(x)
        x = x.view(B, T, H * W).transpose(-1, -2) # (B, F, H * W) -> (B, H * W, F)
        ...
        # (B, H * W, F) -> (B, F, H, W)
        x = x.transpose(-1, -2).view(B, T, H, W)
...
class UNET(nn.Module):
    def __init__(self):
        ...
        self.decoders = nn.ModuleList([
            ...
            # it is double the input compared to the encoders because there are skip connection from the encoder to the decoder
